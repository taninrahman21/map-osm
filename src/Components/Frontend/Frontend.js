...
const Frontend = ({ attributes }) => {
  ...
  const { controlPosition, showPrintButton, showMarkerText, showDirectionFromYourLocation, getLocationControlPosition, showLocationDetailsOnClick } = mapOptions;
  ...
  const [checkCurrentLocation, setCheckCurrentLocation] = useState(currentLocation.showedCurrentLocation);
  ...
  useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current, { fullscreenControl: true, fullscreenControlOptions: { position: controlPosition } }).setView([latitude || currentLocation.lat, longitude || currentLocation.lon], newZoom);
    mapRef.current = map;
    ...
    if (checkCurrentLocation) {
      ...
      map.setView([currentLocation.lat, currentLocation.lon], newZoom);
      L.marker([currentLocation.lat, currentLocation.lon], { icon: getMarkerIcon(iconContent, iconColor) }).addTo(map).bindPopup("You are here :)").openPopup();
    } else {
      map.setView([latitude || currentLocation.lat, longitude || currentLocation.lon], newZoom);
    }
    ...
    // Map Click Option
    if (showLocationDetailsOnClick) {
      map.on('click', function (e) {
        const { lat, lng } = e.latlng;

        setCheckCurrentLocation(false);
  ...
  }, [fromLocationMarkerColor, toLocationMarkerColor, currentLocationMarkerColor, defaultMarkerColor, markerIconSize, currentLocation, markerText, searchQuery, newZoom, latitude, longitude, fromLocation, mapOptions, mapLayer, routeLineColor]);
