...
const MapOsm = compose(withSelect((select) => { return { device: select("core/edit-post").__experimentalGetPreviewDeviceType()?.toLowerCase() } }))(({ attributes, setAttributes, device }) => {
  ...
  const { controlPosition, showPrintButton, showMarkerText, showDirectionFromYourLocation, getYourLocation, getLocationControlPosition, showLocationDetailsOnClick, setSearchLocationOnDblClick } = mapOptions;
  ...
  useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current, { fullscreenControl: true, fullscreenControlOptions: { position: controlPosition } }).setView([latitude || fromLocation.lat, longitude || fromLocation.lon], zoom);
    mapRef.current = map;
    ...
    `;
        ...
        getLocationContainer.addEventListener('click', () => {
          ...
          const marker = L.marker([currentLocation.lat, currentLocation.lon], { icon: getCurrentLocationIcon(iconContent, iconColor) }).addTo(map);
    ...
    if (setSearchLocationOnDblClick) {
        ...
        fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`)
          ...
          .then(data => {
            ...
            if (data) {
              setAttributes({
                mapOsm: produce(mapOsm, draft => {
                  draft.latitude = data.lat;
                  draft.longitude = data.lon;
                  draft.toLocation.lat = data.lat;
                  draft.toLocation.lon = data.lon;
                  draft.toLocation.locationName = data.display_name;
                  draft.markerText = data.display_name;
